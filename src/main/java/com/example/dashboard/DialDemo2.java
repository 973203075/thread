...
public class DialDemo2 {
    public static void main(String[] args){
        dashboard("菠菜(合格率)", 110, 25, 15);

    }

    private static void dashboard(String name, Number pointValue,int percentageFontSize,
        ...
        if(pointValue.intValue() < 0){
            pointValue = 0;
        }
        if(pointValue.intValue() > 100){
            pointValue = 100;
        }
        dataset.setValue(pointValue);
        /**
         *  获取图表区域对象
         *
        ...
        //  设置显示在表盘中央位置的信息
        DialTextAnnotation dialtextannotation = new DialTextAnnotation(pointValue + "%");
...
//        dialplot.addLayer(dialvalueindicator);
        ...
        try {
            ChartUtilities.saveChartAsJPEG(file, jfreechart, 400, 400);
          // ChartUtilities.writeB
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
